        ...
        <div class="panel">
            <div class="sm:flex sm:justify-between gap-4">
                ...
                <div class="flex sm:flex-row flex-col sm:items-center sm:gap-3 gap-4 w-full sm:w-auto">
                    ...
                    <template v-if="userStore.hasAccess('subscriptions.orders.create')">
                ...
                >
                    ...
                    <template #actions="data">
                        <div class="flex justify-center gap-3">
                            <!-- <template v-if="userStore.hasAccess('subscriptions.orders.index')"> -->
                                <button type="button" class="btn btn-sm btn-info" v-tippy:details  @click="redirectToShow(data.value.id)">
                                ...
                                <tippy target="details">{{ $t('details') }}</tippy>
                            <!-- </template> -->
                            <template v-if="userStore.hasAccess('subscriptions.orders.edit')">
                                <div v-if="data.value.status === 1 && data.value.payment_total === 0">
                                    <button type="button" class="btn btn-sm btn-primary" v-tippy:edit  @click="redirectToEdit(data.value.id)">
                                        <svg width="20" height="20" viewBox="0 0 24 24" fill="none" xmlns="http://www.w3.org/2000/svg" class="w-4.5 h-4.5">
                                            <path
                                ...
                                <div v-else class="w-[40px]"></div>
                            </template>

                            <template v-if="userStore.hasAccess('subscriptions.orders.destroy')">
                                <div v-if="data.value.status === 1 && data.value.payment_total === 0">
...
<script lang="ts" setup>
    ...
    const cols = ref([
        ...
        { field:'company.name', title: 'Generado a', hide: false, sort: false},
        { field:'user.full_name', title: 'Generado por', hide: false, sort: false},
    ...
    const getData = async () => {
        loading.value = true;
        try {
            const response = await api.get(`subscriptions/v1/orders?filter={"search":"${params.search}",
            "company_id":[${companyStore.companiesSelect}],
            "order":{"field":"${params.sort_column}","way":"${params.sort_direction}"}}
            &page=${params.current_page}&take=${params.pagesize}`);
