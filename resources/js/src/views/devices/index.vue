        ...
        <button type="button" class="btn btn-outline-info btn-sm d-flex align-items-center" @click="syncData()">
            ...
            <template #engine_status="data">
                <div v-if="data.value?.device_data">
                    <span class="badge" :class="data.value.device_data.engine_status ? 'badge-outline-success':'badge-outline-danger'">{{data.value.device_data.engine_status ? 'Encendido':'Apagado' }}</span>
                </div>
            </template>
        </vue3-datatable>
        </div>
